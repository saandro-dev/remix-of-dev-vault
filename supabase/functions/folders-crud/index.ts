  ...
  try {
    ...
    switch (action) {
      case "list": {
        ...
        if (!project_id) return createErrorResponse(ERROR_CODES.VALIDATION_ERROR, "Missing project_id", 422);
      ...
      case "create": {
        ...
        if (!project_id || !name) {
          return createErrorResponse(ERROR_CODES.VALIDATION_ERROR, "Missing project_id or name", 422);
        }
        const { data, error } = await client
          .from("key_folders")
          .insert({
