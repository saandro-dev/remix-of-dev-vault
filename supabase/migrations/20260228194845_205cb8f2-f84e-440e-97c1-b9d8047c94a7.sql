...
-- Must drop functions first because return type is changing (adding ai_metadata column)
DROP FUNCTION IF EXISTS public.query_vault_modules(text,text,text,text[],integer,integer,integer,text);
DROP FUNCTION IF EXISTS public.hybrid_search_vault_modules(text,text,text,text,text[],integer,double precision,double precision);
DROP FUNCTION IF EXISTS public.get_visible_modules(uuid,text,text,text,text,integer,integer);

-- Recreate query_vault_modules with ai_metadata
...
-- Recreate hybrid_search_vault_modules with ai_metadata
...
-- Recreate get_visible_modules with ai_metadata
CREATE OR REPLACE FUNCTION public.get_visible_modules(
  p_user_id uuid,
  p_scope text DEFAULT 'owned'::text,
  p_domain text DEFAULT NULL::text,
  p_module_type text DEFAULT NULL::text,
  p_query text DEFAULT NULL::text,
