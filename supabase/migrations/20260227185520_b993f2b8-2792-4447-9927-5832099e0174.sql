...
-- Drop functions whose return type changed (adding usage_hint column)
...
-- 4. get_vault_module: add usage_hint to SELECT
CREATE OR REPLACE FUNCTION public.get_vault_module(p_id uuid DEFAULT NULL::uuid, p_slug text DEFAULT NULL::text)
...
-- 6. get_visible_modules: add usage_hint
CREATE OR REPLACE FUNCTION public.get_visible_modules(p_user_id uuid, p_scope text DEFAULT 'owned'::text, p_domain text DEFAULT NULL::text, p_module_type text DEFAULT NULL::text, p_query text DEFAULT NULL::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, title text, description text, domain text, module_type text, language text, tags text[], saas_phase integer, phase_title text, why_it_matters text, usage_hint text, code_example text, source_project text, validation_status text, visibility text, related_modules uuid[], created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
