...
-- 1. Add p_group parameter to query_vault_modules
...
-- 2. Create vault_usage_events table
CREATE TABLE IF NOT EXISTS public.vault_usage_events (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  event_type text NOT NULL,
  module_id uuid REFERENCES public.vault_modules(id) ON DELETE SET NULL,
  query_text text,
...
ALTER TABLE public.vault_usage_events ENABLE ROW LEVEL SECURITY;

-- RLS: only service role can insert (fire-and-forget from Edge Functions)
CREATE POLICY "Service role full access on vault_usage_events"
  ON public.vault_usage_events
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);

-- Admin/owner can read analytics
CREATE POLICY "Admin can view usage events"
  ON public.vault_usage_events
  FOR SELECT
  TO authenticated
...
CREATE INDEX IF NOT EXISTS idx_vault_usage_events_type_created
  ON public.vault_usage_events (event_type, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_vault_usage_events_module
  ON public.vault_usage_events (module_id)
