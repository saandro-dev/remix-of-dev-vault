...
RETURNS TABLE(
  ...
  validation_status text, related_modules uuid[], created_at timestamp with time zone,
  updated_at timestamp with time zone, relevance_score real, difficulty text,
  estimated_minutes integer, ai_metadata jsonb, total_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
